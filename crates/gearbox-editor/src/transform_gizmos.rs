...
impl GizmoMode {
    /// Advance to the next mode, honouring the user-toggleable
    /// `enabled` mask. If the current mode is disabled (e.g. the user
    /// just turned off Scale while in Scale), this also picks the
    /// nearest enabled mode — no intermediate "vanished" frame.
    fn next(self, enabled: &GizmoModesEnabled) -> Self {
        // Hard failsafe: if nothing is enabled, stay put rather than
        // loop forever. UI guarantees at least one is always on.
        if !enabled.any() {
            return self;
        }
        let order = [GizmoMode::Translate, GizmoMode::Rotate, GizmoMode::Scale];
        let start = order.iter().position(|m| *m == self).unwrap_or(0);
...
/// Per-mode enable mask. Lets the user hide modes they don't care
/// about (e.g. turn off Scale on vehicles where scaling the chassis
/// isn't meaningful). Tab only cycles between enabled modes.
...
impl Default for GizmoModesEnabled {
    fn default() -> Self {
        Self { translate: true, rotate: true, scale: true }
    }
}

impl GizmoModesEnabled {
    pub fn has(&self, mode: GizmoMode) -> bool {
        match mode {
...
// match transform-gizmo's chunkier, more readable proportions now that
// `gizmo_material::GizmoMaterial` draws everything on top of the world
// (thin lines read worse when they can't hide in occlusion shadow).
const BASE_SHAFT_RADIUS_W: f32 = 0.028;  // was 0.02
const BASE_TIP_RADIUS_W:   f32 = 0.085;  // was 0.06
const BASE_TIP_LEN_W:      f32 = 0.22;   // was 0.15
const BASE_CUBE_EDGE_W:    f32 = 0.07;   // was 0.05
...
/// Tab key cycles modes (skipping any disabled in
/// [`GizmoModesEnabled`]). Swallowed when egui has keyboard focus.
///
/// Also snaps away from a disabled mode every frame: toggling Scale
/// off while actively in Scale immediately falls back to the nearest
/// enabled mode (Translate or Rotate) rather than leaving a stale
/// "mode: Scale but no handles visible" frame.
pub fn cycle_gizmo_mode(
    keys: Res<ButtonInput<KeyCode>>,
    enabled: Res<GizmoModesEnabled>,
    mut mode: ResMut<GizmoMode>,
    mut contexts: EguiContexts,
) {
    if !enabled.has(*mode) && enabled.any() {
        *mode = mode.next(&enabled);
    }
...
pub fn setup_transform_gizmos(
    ...
    // Placeholder dimensions — the real per-selection sizes land on
    // the first `regenerate_gizmo_meshes` tick. Resolutions bumped to
    // 32 / 128 so edges read clean when the depth-test-off material
    // blows them up close to the camera without hiding imperfections
    // in nearby geometry.
    let shaft_mesh = meshes.add(Cylinder::new(0.01, SHAFT_LEN).mesh().resolution(32).build());
    let tip_mesh   = meshes.add(Cone::new(0.02, 0.05).mesh().resolution(32).build());
...
pub fn hover_transform_gizmos(
    ...
    'pick: {
        ...
        let Ok((camera, cam_tr, projection)) = cameras.single() else { break 'pick };
    ...
    if let Some(prev) = hovered.0 {
        if let Ok((_, h)) = q.get(prev) {
            if let Some(m) = materials.get_mut(&h.material) {
                m.base.emissive = LinearRgba::new(h.base_rgb[0], h.base_rgb[1], h.base_rgb[2], 1.0)
                    * EMISSIVE_BASE;
            }
        }
    }
...
// ── Mesh regeneration ──────────────────────────────────────────────
//
// With the screen-space reach (see `update_transform_gizmos`), the
// parent's `Transform::scale = reach` grows/shrinks with camera
// distance so the gizmo holds a constant pixel size. The mesh radii
// here are pure unit-space fractions of that reach (e.g.
// `BASE_SHAFT_RADIUS_W = 0.028` = "2.8 % of the gizmo's overall
// size"), multiplied only by the user slider — never by reach. Once
// the parent scales by `reach`, every dimension ends up at the same
// pixel count at every camera distance.
//
// Runs only when the slider actually moves — tracked via a `Local`
// `f32` — so mesh buffers aren't re-uploaded every frame even as the
// camera orbits / zooms.
pub fn regenerate_gizmo_meshes(
    ...
    mut last_slider: Local<Option<f32>>,
) {
    let Some(handles) = handles else { return };
    let Some(_id) = selection.vehicle else { return };
    let _ = sim;

    let slider = gizmo_scale.0;

    // Skip if the slider hasn't moved since last pass.
    if let Some(prev) = *last_slider {
        if (prev - slider).abs() < 0.0005 {
            return;
        }
    }
    *last_slider = Some(slider);

    // Pure unit-space radii — the parent's screen-space `scale = reach`
    // blows them up to the pixel count the slider picks.
    let shaft_r = BASE_SHAFT_RADIUS_W * slider;
    let tip_r   = BASE_TIP_RADIUS_W   * slider;
    let tip_len = BASE_TIP_LEN_W      * slider;
    let cube_e  = BASE_CUBE_EDGE_W    * slider;
    let ring_m  = BASE_RING_MINOR_W   * slider;

    // ── Replace mesh data in-place ──
    // Resolutions match the initial setup at the top of this file
    // (32 / 128 / 12) — the always-on-top material means any
    // tessellation stair-step is visible right at the camera plane,
    // so we can't afford the old 16-segment coarse fallback.
    if let Some(m) = meshes.get_mut(&handles.shaft) {
        *m = Cylinder::new(shaft_r, SHAFT_LEN).mesh().resolution(32).build();
    }
    if let Some(m) = meshes.get_mut(&handles.tip) {
        *m = Cone::new(tip_r, tip_len).mesh().resolution(32).build();
    }
