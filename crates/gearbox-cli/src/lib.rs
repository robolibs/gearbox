...
//!
//! Eleven command groups; `run()` only parses and dispatches. Anything that
//! is not a core group runs `gearbox-<name>` from `PATH`, git-style. This
//! is a library so the merged `gearbox` binary (`bin/gearbox`) can call
//! straight into it for every argv shape but its own sim-launch one.
...
/// Parse the process's real argv and dispatch; returns the process exit code.
pub fn run() -> i32 {
    let cli = Cli::parse();
    match dispatch(cli) {
        Ok(()) => 0,
