...
impl Default for WeatherSettings {
    fn default() -> Self {
        ...
        Self {
            ...
            // Far enough to see a country from the air. At forty-five
            // kilometres everything past the next valley washes out to sky
            // colour, so from any height the land is one flat pale sheet — no
            // coast, no hills, no distance, whatever is actually drawn out
            // there. Haze belongs on the horizon, not on the whole view.
            haze_visibility_km: angle("GEARBOX_HAZE_KM", 60.0).clamp(0.5, 600.0),
            clouds: CloudsConfig {
