...
pub(crate) fn update_skybox_transform(
    camera: Single<(&GlobalTransform, &Projection), (With<CloudsCamera>, Without<SkyboxPlane>)>,
    mut skybox: Query<(&mut Transform, &mut GlobalTransform, &SkyboxPlane)>,
) {
    let Projection::Perspective(projection) = camera.1 else {
        return;
    };
    let scale = projection.far * 0.45;
    for (mut transform, mut global, plane) in &mut skybox {
