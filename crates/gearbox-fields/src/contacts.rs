...
pub struct WheelContact {
    ...
    pub direction: Vec2,
    pub width: f32,
    /// Physical footprint length; absent values use the field default.
...
impl WheelContact {
    pub fn footprint_length(&self, fallback: f32) -> f32 {
        self.length.filter(|v| v.is_finite() && *v >= 0.0).unwrap_or(fallback)
    }
}
...
fn explicit_wheel_footprint_overrides_field_default() {
    let mut wheel = WheelContact {
        position: Vec3::ZERO, direction: Vec2::X, width: 0.4, length: None,
        scrub: 0.0, travelled: 0.0, anchor: Vec2::ZERO, centreline: Vec2::ZERO,
    };
