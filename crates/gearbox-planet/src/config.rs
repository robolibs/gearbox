...
impl PlanetConfig {
    ...
    pub fn earth(radius: f32) -> Self {
        // Depth is what the node budget is spent on. At 16 the leaves are 150 m
        // across an Earth-sized globe — field scale — and the fourteen levels
        // between the pinned coarse floor and those leaves cost far more than
        // `MAX_VISIBLE` nodes near the camera, so the selector drops the rest
        // of the sphere and the globe shows holes everywhere but underfoot.
        // The ground a machine actually stands on is the simulated square, not
        // this; the globe only has to be right from a distance, so it is given
        // the depth that covers all of itself rather than the depth that would
        // resolve one field.
        const DEPTH: u8 = 12;
        Self {
            radius,
            ...
            height_amp: 2_400.0,
            max_depth: DEPTH,
            // Wider bands than the default: a tile that is going to be needed
            // is asked for further out, so it is baked before it is looked at
            // rather than popping in once it is already underfoot.
            lod0_range: leaf_size(radius, DEPTH) * 8.0,
            atlas_layers: ATLAS_LAYERS,
