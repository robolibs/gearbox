...
fn terrain_height(dir: vec3<f32>, seed: vec3<f32>, freq: f32) -> f32 {
    let p = dir * freq + seed;
    let continents = fbm(p, 5);
    let mountain_mask = smoothstep(0.02, 0.45, continents + 0.3 * fbm(p * 2.7 + 31.0, 3));
    let mountains = ridged(p * 6.0 + vec3(17.0, 5.0, 9.0), 12) * 2.0 - 1.0;
    let detail = fbm(p * 40.0 + vec3(3.0, 1.0, 7.0), 8);
    var h = continents * 0.65
        + mountain_mask * max(mountains, -0.2) * 0.75
        + detail * 0.06
        - 0.08;
    return clamp(h, -1.0, 1.5);
}
