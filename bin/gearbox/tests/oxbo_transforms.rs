...
fn oxbo_bunker_stays_in_its_closed_blender_pose() {
    ...
    let stage = openusd::Stage::builder()
        .resolver(
            usd_schema::third_party::resolver::StripMetadataResolver::with_search_paths(vec![
                assets.clone(),
            ]),
        )
        .open(assets.join("oxbo.usd").to_str().unwrap())
        .unwrap();
    ...
    for path in [
        ...
        let t = usd_schema::xform::read_transform(&stage, &path)
            .unwrap()
            .unwrap();
