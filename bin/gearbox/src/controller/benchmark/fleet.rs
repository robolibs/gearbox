...
impl Fixture {
    fn load_fleet(path: &Path, count: usize) -> Self {
...
"#
        ...
        Self::from_stage(&stage, FixtureKind::Kubota, count)
    }
...
fn imported_kubota_fleet() {
    ...
    for count in [1, 2, 4] {
        let mut fixture = Fixture::load_fleet(Path::new(&asset), count);
        ...
        for active in phases {
            ...
            let sleeping = physics
                ...
                .count();
            eprintln!(
                "fleet timing machines={count} active={active} sleeping={sleeping}/{} median_ms={:.6} p95_ms={:.6} p99_ms={:.6} speed_ranges={speed_ranges:?} renderer=none controllers=enabled outside-timing",
                physics.bodies().len(),
                samples[600],
                samples[1140],
                samples[1188]
            ...
            for (index, machine) in fixture.fleet.iter().enumerate() {
                fixture.verify_machine(machine.chassis, &machine.wheels, 1.8, index < active);
            }
        }
    }
}
...
fn imported_kubota_fleet_trace() {
    ...
    let mut fixture = Fixture::load_fleet(Path::new(&asset), 4);
    for _ in 0..600 {
        fixture.tick();
    }
