...
fn travel(
    ...
    *started = true;
    ...
    let occupied = physics.bodies().into_iter().filter_map(|id| physics.body(id)).any(|body| {
...
fn reanchor_machines(
    ...
    use crate::physics::backend::{DQuat as Rotation, DVec3 as Vector};
    const TOGETHER_M: f64 = 1_000.0;
    let strayed = physics.bodies().into_iter().filter_map(|id| physics.body(id)).find_map(|body| {
    ...
    let mut poses = Vec::with_capacity(moved.len());
    let mut velocities = Vec::with_capacity(moved.len());
    for entity in &moved {
        let Some(&handle) = physics.entity_to_body.get(entity) else { continue };
        ...
        poses.push((handle, pose));
        velocities.push((handle, linvel, angvel));
    }
    if let Err(error) = physics.set_body_poses(&poses, false) {
        warn!("globe: machine reanchor rejected: {error}");
        return;
    }
    for (handle, linvel, angvel) in velocities {
