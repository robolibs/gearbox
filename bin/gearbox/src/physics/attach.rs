...
pub fn attach_physics_to_prim(
    ...
    if let Ok(Some(mass)) = ph::read_mass(stage, path) {
        world.entity_mut(entity).insert(UsdMass {
            ...
            // A zero quaternion (unset in some exporters) would make the
            // inertia frame NaN; only a real rotation counts.
            principal_axes: mass.principal_axes.and_then(|q| {
                let q = quat_from_usd_wxyz(q);
...
fn collider_shape_from_prim(stage: &Stage, path: &Path, _meta: &StageMeta) -> UsdColliderShape {
    ...
    let type_name = stage.type_name(path).ok().flatten().unwrap_or_default();
...
fn has_mesh_descendant(stage: &Stage, root: &Path) -> bool {
    let Ok(children) = stage.prim_children(root) else {
        return false;
    };
...
fn read_rel_first(stage: &Stage, prim: &Path, rel_name: &str) -> Option<String> {
    ...
    let targets = prim.relationship(rel_name).targets().ok()?;
    targets.into_iter().next().map(|p| p.as_str().to_string())
}
