...
pub fn find_physics_prims(stage: &Stage) -> Result<PhysicsPrims> {
    ...
    stage.traverse(Default::default(), |path: &Path| {
...
fn read_attr_value(stage: &Stage, prim: &Path, name: &str) -> Result<Option<Value>> {
    Ok(crate::usd_ext::authored_value(stage, prim, name))
}
...
fn read_vec3f(stage: &Stage, prim: &Path, name: &str) -> Result<Option<[f32; 3]>> {
    Ok(match read_attr_value(stage, prim, name)? {
        Some(Value::Vec3f(a)) => Some([a.x, a.y, a.z]),
        Some(Value::Vec3d(a)) => Some([a.x as f32, a.y as f32, a.z as f32]),
        _ => None,
    })
}

/// Quaternions come back as `[w, x, y, z]`, the USD authoring order.
fn read_quatf(stage: &Stage, prim: &Path, name: &str) -> Result<Option<[f32; 4]>> {
    Ok(match read_attr_value(stage, prim, name)? {
        Some(Value::Quatf(q)) => Some([q.w, q.x, q.y, q.z]),
        Some(Value::Quatd(q)) => Some([q.w as f32, q.x as f32, q.y as f32, q.z as f32]),
        _ => None,
    })
}
