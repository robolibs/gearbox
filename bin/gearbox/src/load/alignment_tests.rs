use super::*;
use crate::physics::backend::{BodyDesc, BodyId, ColliderDesc, JointDesc, JointKind, Shape};
use crate::physics::PhysicsWorld;

fn fixture(reverse: bool) -> (App, Entity, [Entity; 3], [BodyId; 3], BodyId) {
    let mut app = App::new();
...
fn terrain_alignment_translates_the_whole_suspension_once() {
    for reverse in [false, true] {
        let (mut app, root, _, ids, other) = fixture(reverse);
...
fn invalid_projected_pose_keeps_the_entire_machine_pending_and_unchanged() {
    let (mut app, root, entities, ids, _) = fixture(false);
