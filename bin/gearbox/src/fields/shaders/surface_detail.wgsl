#import bevy_pbr::{pbr_types::PbrInput, pbr_functions::apply_pbr_lighting}
...
fn surface_lighting(input: PbrInput, canopy: f32) -> vec4<f32> {
    var result = apply_pbr_lighting(input) * (1.0 - canopy);
    let n = input.N;
    ...
    for (var i = 0u; i < 4u; i += 1u) {
        ...
        result += apply_pbr_lighting(leaf) * (canopy * 0.25);
    }
    return result;
}
...
fn surface_relief(world_xz: vec2<f32>, normal: vec3<f32>, footprint: f32) -> vec3<f32> {
    ...
    return normalize(normal + vec3<f32>(-dx, 0.0, -dz) * (0.7 * resolved));
}
