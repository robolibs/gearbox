...
-- session, regardless of what real hardware is present. So this has to run first, and
-- nix_develop() below has to be told `impure = true` — `builtins.getEnv` always answers
-- "" in a pure evaluation, whatever a same-script `oslo.env.set()` just did.
--
-- Pure Lua/`oslo.fs`, no shelling out to `lspci`/`sh` at all: `/sys/bus/pci/devices/*/vendor`
-- is the kernel's own PCI bus enumeration, always readable with no external command, and
-- reflects hardware presence regardless of whether the nvidia kernel module happens to be
-- loaded at this exact instant.
--
...
if nvidia_version then
  ...
  -- where the compositor itself runs on the integrated one.
